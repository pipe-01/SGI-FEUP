    ...
    parsePrimitives(primitivesNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if (primitiveType == 'rectangle') {
                ...
                if(patchChildren.length !== (degreeU + 1) * (degreeV + 1)){
                ...
                var controlPoints = [];
                var pointsAux = [];
                for (let i = 0; i < patchChildren.length; i++) {
                    let point = this.parseCoordinates3D(patchChildren[i]);
                    controlPointsAux.push(point[0], point[1], point[2], 1.0);
                }

                let indexPoint = 0;
                for (let u = 0; u < degreeU + 1; u++) {
                    for (let v = 0; v < degreeV + 1; v++) {
                        pointsAux.push([ controlPointsAux[indexPoint], controlPointsAux[indexPoint+1], controlPointsAux[indexPoint+2], controlPointsAux[indexPoint+3] ]);
                        indexPoint +=4;
                    }
                    controlPoints.push(pointsAux);
                    pointsAux = [];
                }
